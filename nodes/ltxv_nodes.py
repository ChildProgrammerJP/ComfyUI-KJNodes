# エラー
# /workspace/ComfyUI/custom_nodes/ComfyUI-KJNodes/nodes/ltxv_nodes.py
# [ERROR] An error occurred while retrieving information for the 'LTXVAddGuideMulti' node.
# Traceback (most recent call last):
#   File "/workspace/ComfyUI/server.py", line 690, in get_object_info
#     out[x] = node_info(x)
#              ^^^^^^^^^^^^
#   File "/workspace/ComfyUI/server.py", line 652, in node_info
#     return obj_class.GET_NODE_INFO_V1()
#           ^^^^^^^^^^^^^^^^^^^^^^^^^^^^
#   File "/workspace/ComfyUI/comfy_api/latest/_io.py", line 1578, in GET_NODE_INFO_V1
#     schema = cls.GET_SCHEMA()
#              ^^^^^^^^^^^^^^^^
#   File "/workspace/ComfyUI/comfy_api/latest/_io.py", line 1707, in GET_SCHEMA
#     schema = cls.FINALIZE_SCHEMA()
#              ^^^^^^^^^^^^^^^^^^^^^
#   File "/workspace/ComfyUI/comfy_api/latest/_io.py", line 1698, in FINALIZE_SCHEMA
#     schema = cls.define_schema()
#              ^^^^^^^^^^^^^^^^^^^
#   File "/workspace/ComfyUI/custom_nodes/ComfyUI-KJNodes/nodes/ltxv_nodes.py", line 28, in define_schema
...
class LTXVAddGuideMulti(LTXVAddGuide):
    ...
    def define_schema(cls):
        options = []
        # DynamicCombo が存在するかチェック
        has_dynamic_combo = hasattr(io, "DynamicCombo")
        ...
        # DynamicComboが使えない環境向けのフォールバック入力
        if has_dynamic_combo:
            dynamic_input = io.DynamicCombo.Input(
                "num_guides",
                options=options,
            ...
            # 代わりにシンプルな数値入力を置く（エラー回避用）
            dynamic_input = io.Int.Input("num_guides", default=1, min=1, max=20)
    ...
    def execute(cls, positive, negative, vae, latent, num_guides) -> io.NodeOutput:
        ...
        # num_guidesが数値（フォールバック時）か辞書（通常時）か判定
        if isinstance(num_guides, dict):
            image_keys = sorted([k for k in num_guides.keys() if k.startswith('image_')])
            data_source = num_guides
        else:
            # フォールバック時は機能制限
        ...
        for img_key in image_keys:
            i = img_key.split('_')[1]
            img = data_source[f"image_{i}"]
            f_idx = data_source[f"frame_idx_{i}"]
            strength = data_source[f"strength_{i}"]
